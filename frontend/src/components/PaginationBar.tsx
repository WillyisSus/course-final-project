...
export default function PaginationBar(props:PagiantionBarProps){
    ...
    const currentShowingItem = useMemo(() => {
        ...
        const lastItemOnPage = Math.min(firstItemOnPage + props.itemsPerPage - 1, props.totalItems)
    ...
    return (
        ...
        <nav className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-8 border-t border-border">
            <span className="text-secondary">Showing {`${currentShowingItem.firstItemOnPage} - ${currentShowingItem.lastItemOnPage}`} of {props.totalItems} events</span>
            <div className="flex items-center gap-2">
                <button onClick={props.onPrev} className="flex hover:cursor-pointer items-center gap-2 px-4 py-2 text-secondary hover:text-text transition-colors duration-200">
                ...
                <div className="flex gap-2">
                    {getPageNumbers().map((item) => (
                        <button key={item} disabled={typeof item === "string"} onClick={() => typeof item === 'number' && props.onPageChange(item)} 
                        className={`px-4 py-2 hover:cursor-pointer ${props.currentPage === item ? "bg-orange-500 text-white" : "bg-white text-black transition-colors duration-200"} rounded-lg font-medium`}>{item}</button>
                    ))} 
                </div>

                <button onClick={props.onNext} className="flex hover:cursor-pointer items-center gap-2 px-4 py-2 text-text hover:text-primary transition-colors duration-200">
                    Next
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
