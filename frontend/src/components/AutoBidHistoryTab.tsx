            ...
            <CardHeader>
                <div className="flex justify-between items-center">
                    <div className="space-y-1">
                        ...
                        <CardDescription>Automatic bidding configurations you have set</CardDescription>
                    </div>
            ...
            <CardContent>
                <div className="rounded-md border">
                    <Table>
                        ...
                        <TableBody>
                            {autoBids.length > 0 ? autoBids.map(item => {
                                const isWinner = item.product.winner_id === userId;
                                const isMaxedOut = !isWinner && Number(item.product.price_current) >= Number(item.max_price);
                                const isSold = item.product.status === 'SOLD';

                                return (
                                    <TableRow key={item.auto_bid_id}>
                                        <TableCell className="font-medium">{item.product.name}</TableCell>
                                        <TableCell className="font-bold">${Number(item.max_price).toLocaleString()}</TableCell>
                                        <TableCell className="text-muted-foreground">${Number(item.product.price_current).toLocaleString()}</TableCell>
                                        <TableCell>
                                            {isSold && isWinner ? (
                                                <Badge className="bg-green-600 gap-1"><CheckCircle className="w-3 h-3" /> Won</Badge>
                                            ) : isSold && !isWinner ? (
                                                <Badge variant="secondary" className="gap-1"><XCircle className="w-3 h-3" /> Lost</Badge>
                                            ) : isWinner ? (
                                                <Badge className="bg-blue-600 gap-1"><CheckCircle className="w-3 h-3" /> Winning</Badge>
                                            ) : isMaxedOut ? (
                                                <Badge variant="destructive" className="bg-red-100 text-red-700 hover:bg-red-200 border-red-200 gap-1"><AlertTriangle className="w-3 h-3" /> Outbid</Badge>
                                            ) : (
                                                <Badge variant="outline" className="text-orange-600 border-orange-200 gap-1"><Clock className="w-3 h-3" /> Active</Badge>
                                            )}
                                        </TableCell>
