...
import { Send, ArrowLeft, Lock, CheckCircle2, PackageCheck, Banknote } from 'lucide-react';
...
const CheckoutPage = () => {
    ...
    const partnerName = isOwner ? receipt?.product.winner?.full_name : receipt?.product.seller?.full_name;
    ...
    const handleUpdateStatus = async (type: 'confirm_payment' | 'confirm_receipt') => {
        if (!receipt) return;
        try {
            const payload: any = {};
    ...
    if (!receipt) return <div className="h-screen flex items-center justify-center">Receipt not found</div>;
            ...
            <header className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">Receipt #{receipt.receipt_id} • {new Date(receipt.created_at).toLocaleDateString()}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    {receipt.confirmed_by_seller && (
                        <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 hidden md:flex gap-1">
                            <CheckCircle2 className="w-3 h-3" /> Payment Confirmed
                        </Badge>
                    )}
                    {receipt.confirmed_by_buyer && (
                        <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 hidden md:flex gap-1">
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-4 h-full ">
                        <Card className="border-0 shadow-lg ring-1 ring-gray-100 flex-1 flex flex-col">
                            ...
                            <CardContent className="space-y-6 pt-6 flex-1">
                                <div className="flex gap-4">
                                    <div className="w-24 h-24 bg-gray-100 rounded-lg overflow-hidden shrink-0 border">
                                        <img 
                                            src={receipt.product.product_images?.[0]?.image_url ? `${receipt.product.product_images[0].image_url}` : "/placeholder.png"} 
                                            alt={receipt.product.name} 
                                            className="w-full h-full object-cover"
                                        />
                                    </div>
                                    <div className="space-y-1">
                                        <h3 className="font-bold text-gray-900 line-clamp-2">{receipt.product.name}</h3>
                                ...
                                <div className="space-y-4">
                                    {/* Step 1: Buyer Paid */}
                                    <div className={`p-3 rounded-lg border flex items-center justify-between ${receipt.paid_by_buyer ? 'bg-green-50 border-green-200' : 'bg-gray-50'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-full ${receipt.paid_by_buyer ? 'bg-green-100' : 'bg-gray-200'}`}>
                                                <Banknote className={`w-4 h-4 ${receipt.paid_by_buyer ? 'text-green-700' : 'text-gray-500'}`} />
                                            </div>
                                            <div className="text-sm">
                                                <p className="font-semibold text-gray-900">Buyer Payment</p>
                                                <p className="text-xs text-muted-foreground">{receipt.paid_by_buyer ? "Funds captured securely" : "Waiting for payment"}</p>
                                            </div>
                                        </div>
                                        {receipt.paid_by_buyer && <CheckCircle2 className="w-5 h-5 text-green-600" />}
                                    </div>

                                    {/* Step 2: Seller Confirmed */}
                                    <div className={`p-3 rounded-lg border flex items-center justify-between ${receipt.confirmed_by_seller ? 'bg-blue-50 border-blue-200' : 'bg-gray-50'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-full ${receipt.confirmed_by_seller ? 'bg-blue-100' : 'bg-gray-200'}`}>
                                                <CheckCircle2 className={`w-4 h-4 ${receipt.confirmed_by_seller ? 'text-blue-700' : 'text-gray-500'}`} />
                                            </div>
                                            <div className="text-sm">
                                                <p className="font-semibold text-gray-900">Seller Confirmation</p>
                                                <p className="text-xs text-muted-foreground">
                                                    {receipt.confirmed_by_seller ? "Payment verified by seller" : "Waiting for seller check"}
                                                </p>
                                            </div>
                                        </div>
                                        {receipt.confirmed_by_seller && <Badge className="bg-blue-600">Confirmed</Badge>}
                                    </div>

                                    {/* Step 3: Buyer Received */}
                                    <div className={`p-3 rounded-lg border flex items-center justify-between ${receipt.confirmed_by_buyer ? 'bg-purple-50 border-purple-200' : 'bg-gray-50'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-full ${receipt.confirmed_by_buyer ? 'bg-purple-100' : 'bg-gray-200'}`}>
                                                <PackageCheck className={`w-4 h-4 ${receipt.confirmed_by_buyer ? 'text-purple-700' : 'text-gray-500'}`} />
                                            </div>
                                            <div className="text-sm">
                                                <p className="font-semibold text-gray-900">Order Completed</p>
                                                <p className="text-xs text-muted-foreground">
                                                    {receipt.confirmed_by_buyer ? "Item received by buyer" : "In transit / Pending"}
                                                </p>
                                            </div>
                                        </div>
                                        {receipt.confirmed_by_buyer && <Badge className="bg-purple-600">Received</Badge>}
                                    </div>
                                </div>
                            ...
                            <CardFooter className="bg-gray-50/50 border-t p-2 flex flex-col gap-3">
                                ...
                                {isOwner && receipt.paid_by_buyer && !receipt.confirmed_by_seller && (
                                    <Button 
                                ...
                                {!isOwner && receipt.confirmed_by_seller && !receipt.confirmed_by_buyer && (
                                    <Button 
                        ...
                        <Card className=" flex flex-col border-0 shadow-lg ring-1 ring-gray-100 overflow-hidden">
                            ...
                            <div className="overflow-y-scroll h-96 bg-slate-50/50 p-6 space-y-4">
                                <div className="flex justify-center">
                                        ...
                                        Transaction room created. Please discuss shipping details here.
                                    </div>
                                </div>
                                {messages.map((msg) => {
                            ...
                            <div className="p-4 bg-white border-t shrink-0">
                                <form onSubmit={handleSendMessage} className="flex gap-2">
                                    <Input 
                                        ...
                                        disabled={isSending}
                                    />
                                    <Button type="submit" size="icon" className="bg-blue-600 hover:bg-blue-700" disabled={isSending}>
